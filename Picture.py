...
class PictureSH(Picture):
    def __init__(self):
        ...
        self.name_eng_font = ImageFont.load_default()
        self.name_eng_color = "black"
        self.name_eng_bg_color = "white"

        self.price_font = ImageFont.load_default()
        self.price_color = "white"
        self.price_bg_color = "red"
    
    def set_name_eng_font(self, font_name : str, font_size : int):
        self.name_eng_font = ImageFont.truetype(font_name, font_size)

    def set_name_eng_bg_color(self, color : str):
        self.name_eng_bg_color = color

    def set_name_eng_color(self, color : str):
        self.name_eng_color = color
    ...
    def draw_image_name(self):
        draw = ImageDraw.Draw(self.pil_image)
        pic_name_size = draw.textsize(self.pic_name, self.name_font)
        pic_name_eng_size = draw.textsize(self.pic_name_eng, self.name_eng_font)
        pic_price_size = draw.textsize(self.pic_price, self.price_font)
        # Draw Chinese name
        draw.rectangle((0, 0, pic_name_size[0], pic_name_size[1]), self.name_bg_color)
        draw.text((0, 0), self.pic_name, self.name_color, font=self.name_font)
        # Draw English name
        draw.rectangle((0, pic_name_size[1], pic_name_eng_size[0], pic_name_size[1]+pic_name_eng_size[1]), self.name_eng_bg_color)
        draw.text((0, pic_name_size[1]), self.pic_name_eng, self.name_eng_color, font=self.name_eng_font)
        # Draw Price
